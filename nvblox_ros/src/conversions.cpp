...
bool RosConverter::colorImageFromImageMessage(
    const sensor_msgs::ImageConstPtr& image_msg, ColorImage* color_image) {
  CHECK_NOTNULL(color_image);

  sensor_msgs::ImageConstPtr imgMsg =
      cv_bridge::CvImage(
          std_msgs::Header(), "rgb8",
  ...
  color_image->populateFromBuffer(
      ...
      reinterpret_cast<const Color*>(&imgMsg->data[0]),
      MemoryType::kDevice);  // &image_msg->data[0]

  return true;
}

void RosConverter::imageMessageFromDepthImage(const DepthImage& depth_image,
...
void RosConverter::distanceMapSliceFromLayer(
  ...
  populateSliceFromLayer(layer, aabb, z_slice_level, voxel_size, kUnknownValue,
                         &image);

  checkCudaErrors(cudaMemcpy(map_slice->data.data(), image.dataPtr(),
