...
bool RosConverter::depthImageFromImageMessage(
  ...
  if (image_msg->encoding != "32FC1" && image_msg->encoding != "16UC1" &&
    ...
    DLOG(ERROR) << "Invalid image encoding: " << image_msg->encoding
                << std::endl;
    return false;
  }
  ...
  if (image_msg->encoding == "32FC1") {
  ...
  } else if (image_msg->encoding == "16UC1" or
             image_msg->encoding == "mono16") {
    ...
    if (kUseCuda) {
      ...
      // Now just thrust it.
      thrust::transform(char_buffer, char_buffer + numel,
      ...
      for (int i = 0; i < numel; i++) {
        float_buffer[i] =
            static_cast<float>(char_buffer[i]) / scale_factor - scale_offset;
        if (!std::isfinite(float_buffer[i])) {
        }
      }
      depth_image->populateFromBuffer(intHeight, intWidth, float_buffer.data(),
                                      MemoryType::kDevice);
    }
  }
  return true;
...
// Convert image to height frame object
bool RosConverter::heightImageFromImageMessage(
    const sensor_msgs::ImageConstPtr& image_msg, HeightImage* height_image,
    const float scale_factor, const float scale_offset) {
  CHECK_NOTNULL(height_image);
  // If the image is a float, we can just copy it over directly.
  // If the image is int16, we need to divide by 1000 to get the correct
  // format for us.
  ...
  if (image_msg->encoding == "32FC1") {
    // Float to float, so this should be a straight-up copy. :)
    height_image->populateFromBuffer(
        image_msg->height, image_msg->width,
        reinterpret_cast<const float*>(&image_msg->data[0]));
  } else if (image_msg->encoding == "16UC1" or
             image_msg->encoding == "mono16") {
    ...
    if (kUseCuda) {
      // Make sure there's enough output space.
      if (height_image->numel() < numel) {
        *height_image = HeightImage(image_msg->height, image_msg->width,
                                    MemoryType::kDevice);
      }
      // Now just thrust it.
      thrust::transform(char_buffer, char_buffer + numel,
                        height_image->dataPtr(), DivideBy1000());
    } else {
      std::vector<float> float_buffer(numel);
      for (int i = 0; i < numel; i++) {
        float_buffer[i] =
      ...
      height_image->populateFromBuffer(intHeight, intWidth, float_buffer.data(),
                                       MemoryType::kDevice);
    }
  }
  return true;
...
// Convert image to semantic frame object
// TODO(gogojjh): if the semanticimage is encoded as bgr8
